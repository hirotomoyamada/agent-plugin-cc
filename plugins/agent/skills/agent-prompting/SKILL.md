...
description: Internal guidance for composing Cursor Agent prompts for coding, review, diagnosis, and research tasks inside the Agent Claude Code plugin
user-invocable: false
---

# Agent Prompting

Use this skill when `agent:agent-rescue` needs to ask Agent for help.

Prompt Agent like an operator, not a collaborator. Keep prompts compact and block-structured with XML tags. State the task, the output contract, the follow-through defaults, and the small set of extra constraints that matter.

Core rules:
- Prefer one clear task per Agent run. Split unrelated asks into separate runs.
- Tell Agent what done looks like. Do not assume it will infer the desired end state.
...
- `<default_follow_through_policy>`: what Agent should do by default instead of asking routine questions.
...
- Write-capable tasks: add `action_safety` so Agent stays narrow and avoids unrelated refactors.
...
- Use `task --resume-last` for follow-up instructions on the same Agent thread. Send only the delta instruction instead of restating the whole prompt unless the direction changed materially.
...
- Ask Agent for brief, outcome-based progress updates only when the task is long-running or tool-heavy.
...
3. Decide whether Agent should keep going by default or stop for missing high-risk details.
