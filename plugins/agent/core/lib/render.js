...
function normalizeReviewFinding(finding, index) {
    const source = finding && typeof finding === "object" && !Array.isArray(finding)
        ? finding
        : {};
    const lineStart = Number.isInteger(source.line_start) && source.line_start > 0
        ? source.line_start
        : null;
...
function formatResumeCommand(config, job) {
    if (!job?.threadId) {
        return null;
    }
    return `${config.cliResumeBinary} resume ${job.threadId}`;
}
function appendActiveJobsTable(config, lines, jobs) {
    lines.push("Active jobs:");
    lines.push(`| Job | Kind | Status | Phase | Elapsed | ${config.displayName} Session ID | Summary | Actions |`);
    lines.push("| --- | --- | --- | --- | --- | --- | --- | --- |");
    for (const job of jobs) {
        const actions = [`${config.slashPrefix}:status ${job.id}`];
        if (job.status === "queued" || job.status === "running") {
            actions.push(`${config.slashPrefix}:cancel ${job.id}`);
        }
        lines.push(`| ${escapeMarkdownCell(job.id)} | ${escapeMarkdownCell(job.kindLabel)} | ${escapeMarkdownCell(job.status)} | ${escapeMarkdownCell(job.phase ?? "")} | ${escapeMarkdownCell(job.elapsed ?? "")} | ${escapeMarkdownCell(job.threadId ?? "")} | ${escapeMarkdownCell(job.summary ?? "")} | ${actions.map((action) => `\`${action}\``).join("<br>")} |`);
    }
}
function pushJobDetails(config, lines, job, options = {}) {
    ...
    if (job.status !== "queued" &&
        ...
        lines.push(`  Stricter review: ${config.slashPrefix}:adversarial-review --wait`);
    }
...
export function renderSetupReport(config, report) {
    const lines = [
...
export function renderReviewResult(config, parsedResult, meta) {
...
export function renderNativeReviewResult(config, result, meta) {
    ...
    if (stdout) {
        ...
        lines.push(`${config.displayName} review completed without any stdout output.`);
    }
    else {
...
export function renderTaskResult(config, parsedResult, _meta) {
...
export function renderStatusReport(config, report) {
    const lines = [
    ...
    if (report.needsReview) {
        ...
        lines.push(`Ending the session will trigger a fresh ${config.displayName} adversarial review and block if it finds issues.`);
    }
    return `${lines.join("\n").trimEnd()}\n`;
}
export function renderJobStatusReport(config, job) {
    const lines = [`# ${config.displayName} Job Status`, ""];
    pushJobDetails(config, lines, job, {
        showCancelHint: true,
...
export function renderStoredJobResult(config, job, storedJob) {
    const threadId = storedJob?.threadId ??
        job.threadId ??
        null;
    ...
    const rawOutput = (typeof storedJob?.result?.rawOutput === "string" &&
...
export function renderCancelReport(config, job) {
    const lines = [
