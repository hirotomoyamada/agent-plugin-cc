...
import { BROKER_BUSY_RPC_CODE, BROKER_ENDPOINT_ENV, AgentAppServerClient, } from "./app-server.js";
...
async function _startThread(client, cwd, options = {}) {
    ...
    if (options.threadName) {
        try {
            ...
            if (!msg.includes("unknown variant") &&
...
export function getSessionRuntimeStatus(env = process.env, cwd = process.cwd()) {
    const endpoint = env?.[BROKER_ENDPOINT_ENV] ??
