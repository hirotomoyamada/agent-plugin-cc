...
import { buildPersistentTaskThreadName, DEFAULT_CONTINUE_PROMPT, findLatestTaskThread, getCodexAuthStatus, getCodexAvailability, getSessionRuntimeStatus, interruptAppServerTurn, parseStructuredOutput, readOutputSchema, runAppServerReview, runAppServerTurn, } from "./lib/codex.js";
...
function normalizeReasoningEffort(effort) {
    ...
    if (!VALID_REASONING_EFFORTS.has(normalized)) {
        throw new Error(`Unsupported reasoning effort "${raw}". Use one of: none, minimal, low, medium, high, xhigh.`);
    }
    return normalized;
}
...
function validateNativeReviewRequest(target, focusText) {
    if (focusText.trim()) {
        throw new Error(`\`/codex:review\` now maps directly to the built-in reviewer and does not support custom focus text. Retry with \`/codex:adversarial-review ${focusText.trim()}\` for focused review instructions.`);
    }
...
async function handleTaskWorker(argv) {
    ...
    await runTrackedJob({ ...storedJob, id: storedJob.id, logFile, workspaceRoot }, () => executeTaskRun({
        ...request,
        onProgress: progress,
    }), { logFile });
}
