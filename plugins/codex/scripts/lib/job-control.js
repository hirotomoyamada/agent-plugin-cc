...
function inferLegacyJobPhase(job, progressPreview = []) {
    ...
    for (let index = progressPreview.length - 1; index >= 0; index -= 1) {
        ...
        if (line.startsWith("starting codex") ||
...
export function resolveResultJob(cwd, reference) {
    ...
    if (active) {
        throw new Error(`Job ${active.id} is still ${coerceString(active.status, "unknown")}. Check /codex:status and try again once it finishes.`);
    }
    if (reference) {
