...
export function createBrokerEndpoint(config, sessionDir, platform = process.platform) {
    if (platform === "win32") {
        const pipeName = sanitizePipeName(`${path.win32.basename(sessionDir)}-${config.paths.socketSuffix}`);
