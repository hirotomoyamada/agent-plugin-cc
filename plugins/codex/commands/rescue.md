...
argument-hint: "[--background|--wait] [--resume|--resume-id <threadId>|--fresh] [--model <model|spark>] [--effort <none|minimal|low|medium|high|xhigh>] [what Codex should investigate, solve, or continue]"
...
- If the request includes `--resume-id <threadId>`, do not ask whether to continue. The user already chose to resume that specific thread.
- If the request includes `--fresh`, do not ask whether to continue. The user already chose.
- Otherwise, before starting Codex, check for a resumable rescue thread from this Claude session by running:

```bash
node "${CLAUDE_PLUGIN_ROOT}/scripts/codex-companion.js" task-resume-candidate --json
```
...
- The subagent is a thin forwarder only. It should use one `Bash` call to invoke `node "${CLAUDE_PLUGIN_ROOT}/scripts/codex-companion.js" task ...` and return that command's stdout as-is.
...
- Leave `--resume`, `--resume-id <threadId>`, and `--fresh` in the forwarded request. The subagent handles that routing when it builds the `task` command.
