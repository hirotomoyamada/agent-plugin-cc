...
function getCurrentSessionId(config, options = {}) {
    return (options.env?.[config.envVars.sessionId] ??
        process.env[config.envVars.sessionId] ??
        null);
}
function filterJobsForCurrentSession(config, jobs, options = {}) {
...
function looksLikeVerificationCommand(line) {
    return /\b(test|tests|lint|build|typecheck|type-check|check|verify|validate|pytest|jest|vitest|cargo test|npm test|pnpm test|yarn test|go test|mvn test|gradle test|tsc|eslint|ruff)\b/i.test(line);
}
function inferLegacyJobPhase(config, job, progressPreview = []) {
    switch (job.status) {
        case "queued":
            return "queued";
        case "cancelled":
    ...
    for (let index = progressPreview.length - 1; index >= 0; index -= 1) {
        ...
        if (line.startsWith(startingPrefix) ||
...
export function enrichJob(config, job, options = {}) {
    ...
    return {
        ...
        phase: enriched.phase ??
            inferLegacyJobPhase(config, enriched, enriched.progressPreview),
    };
}
export function readStoredJob(config, workspaceRoot, jobId) {
...
function matchJobReference(config, jobs, reference, predicate = () => true) {
    ...
    throw new Error(`No job found for "${reference}". Run ${config.slashPrefix}:status to list known jobs.`);
}
export function buildStatusSnapshot(config, cwd, options) {
    const workspaceRoot = resolveWorkspaceRoot(cwd);
    const stateConfig = getConfig(config, workspaceRoot);
    const jobs = sortJobsNewestFirst(filterJobsForCurrentSession(config, listJobs(config, workspaceRoot), options));
...
export function buildSingleJobSnapshot(config, cwd, reference, options = {}) {
    const workspaceRoot = resolveWorkspaceRoot(cwd);
    const jobs = sortJobsNewestFirst(listJobs(config, workspaceRoot));
    const selected = matchJobReference(config, jobs, reference);
    if (!selected) {
        throw new Error(`No job found for "${reference}". Run ${config.slashPrefix}:status to inspect known jobs.`);
    }
    return {
...
export function resolveResultJob(config, cwd, reference) {
    const workspaceRoot = resolveWorkspaceRoot(cwd);
    ...
    const selected = matchJobReference(config, jobs, reference, (job) => job.status === "completed" ||
        job.status === "failed" ||
        job.status === "cancelled");
    if (selected) {
        return { job: selected, workspaceRoot };
    }
    const active = matchJobReference(config, jobs, reference, (job) => job.status === "queued" || job.status === "running");
    if (active) {
        throw new Error(`Job ${active.id} is still ${coerceString(active.status, "unknown")}. Check ${config.slashPrefix}:status and try again once it finishes.`);
    }
    if (reference) {
        throw new Error(`No finished job found for "${reference}". Run ${config.slashPrefix}:status to inspect active jobs.`);
    }
    throw new Error(`No finished ${config.displayName} jobs found for this repository yet.`);
}
export function resolveCancelableJob(config, cwd, reference, options = {}) {
    const workspaceRoot = resolveWorkspaceRoot(cwd);
    const jobs = sortJobsNewestFirst(listJobs(config, workspaceRoot));
    ...
    const sessionScopedActiveJobs = filterJobsForCurrentSession(config, activeJobs, options);
    if (sessionScopedActiveJobs.length === 1) {
        return { job: sessionScopedActiveJobs[0], workspaceRoot };
    }
    if (sessionScopedActiveJobs.length > 1) {
        throw new Error(`Multiple ${config.displayName} jobs are active. Pass a job id to ${config.slashPrefix}:cancel.`);
    }
    if (getCurrentSessionId(config, options)) {
        throw new Error(`No active ${config.displayName} jobs to cancel for this session.`);
    }
