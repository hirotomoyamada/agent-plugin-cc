...
export const createJobLogFile = (
  ...
  jobId: string,
  title?: string,
) => core.createJobLogFile(AGENT_PROVIDER, workspaceRoot, jobId, title)
...
) => core.createJobRecord(AGENT_PROVIDER, base, { env: options.env })

export const createJobProgressUpdater = (
  workspaceRoot: string,
  jobId: string,
) => core.createJobProgressUpdater(AGENT_PROVIDER, workspaceRoot, jobId)
...
export const createProgressReporter = (options: ProgressReporterOptions = {}) =>
  core.createProgressReporter(AGENT_PROVIDER, options as never)
...
) => core.runTrackedJob(AGENT_PROVIDER, job, runner, options)
