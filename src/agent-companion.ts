...
function outputCommandResult(payload: any, rendered: string, asJson: boolean): void {
...
function parseCommandInput(
  argv: string[],
  config: any = {}
): { options: Record<string, string | boolean>; positionals: string[] } {
...
function resolveCommandCwd(options: Record<string, any> = {}): string {
  return options.cwd ? path.resolve(process.cwd(), String(options.cwd)) : process.cwd();
}
...
async function buildSetupReport(cwd: string, actionsTaken: string[] = []): Promise<any> {
  ...
  if (!agentStatus.available) {
    nextSteps.push("Install Cursor IDE from https://www.cursor.com to get the `agent` CLI.");
  }
  if (agentStatus.available && !authStatus.loggedIn) {
    nextSteps.push("Run `!agent login`.");
  }
  if (!config.stopReviewGate) {
    nextSteps.push("Optional: run `/agent:setup --enable-review-gate` to require a fresh review before stop.");
  }
...
function buildNativeReviewTarget(target: any): { type: string; branch?: string } | null {
...
function validateNativeReviewRequest(target: any, focusText: string): { type: string; branch?: string } {
  if (focusText.trim()) {
    throw new Error(
...
function findLatestResumableTaskJob(jobs: any[]): any | null {
  return (
    jobs.find(
      (job: any) => job.jobClass === "task" && job.threadId && job.status !== "queued" && job.status !== "running"
    ) ?? null
  );
}

async function waitForSingleJobSnapshot(cwd: string, reference: string, options: any = {}): Promise<any> {
  const timeoutMs = Math.max(0, Number(options.timeoutMs) || DEFAULT_STATUS_WAIT_TIMEOUT_MS);
  const pollIntervalMs = Math.max(100, Number(options.pollIntervalMs) || DEFAULT_STATUS_POLL_INTERVAL_MS);
  const deadline = Date.now() + timeoutMs;
  let snapshot = buildSingleJobSnapshot(cwd, reference);

  while (isActiveJobStatus(String(snapshot.job.status)) && Date.now() < deadline) {
...
async function resolveLatestTrackedTaskThread(cwd: string, options: any = {}): Promise<{ id: string } | null> {
  const workspaceRoot = resolveWorkspaceRoot(cwd);
  const sessionId = getCurrentClaudeSessionId();
  const jobs = sortJobsNewestFirst(listJobs(workspaceRoot)).filter((job: any) => job.id !== options.excludeJobId);
  const visibleJobs = filterJobsForCurrentClaudeSession(jobs);
  const activeTask = visibleJobs.find(
    (job: any) => job.jobClass === "task" && (job.status === "queued" || job.status === "running")
  );
  if (activeTask) {
    throw new Error(`Task ${activeTask.id} is still running. Use /agent:status before continuing it.`);
  }
...
async function executeTaskRun(request: any): Promise<any> {
  ...
  if (request.resumeId) {
    ...
    if (!latestThread) {
      throw new Error("No previous Agent task thread was found for this repository.");
    }
...
function buildReviewJobMetadata(reviewName: string, target: any): { kind: string; title: string; summary: string } {
  return {
...
function createTrackedProgress(job: any, options: any = {}): { logFile: string; progress: any } {
...
function buildTaskJob(workspaceRoot: string, taskMetadata: { title: string; summary: string }, write: boolean): any {
...
function enqueueBackgroundTask(cwd: string, job: any, request: any): { payload: any; logFile: string } {
...
async function handleTaskWorker(argv: string[]): Promise<void> {
  ...
  await runTrackedJob(
    { ...storedJob, workspaceRoot, logFile } as any,
...
async function handleStatus(argv: string[]): Promise<void> {
  ...
  outputResult(renderStatusPayload(report, Boolean(options.json)), Boolean(options.json));
}
