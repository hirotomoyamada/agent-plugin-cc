...
export type ReviewTarget =
  | { mode: "branch"; label: string; baseRef: string; explicit: boolean }
  | { mode: "working-tree"; label: string; explicit: boolean };
...
function git(cwd: string, args: string[], options: RunCommandOptions = {}): CommandResult {
  return runCommand("git", args, { cwd, ...options });
}

function gitChecked(cwd: string, args: string[], options: RunCommandOptions = {}): CommandResult {
...
export function resolveReviewTarget(cwd: string, options: ReviewTargetOptions = {}): ReviewTarget {
  ...
  if (baseRef) {
    return { mode: "branch" as const, label: `branch diff against ${baseRef}`, baseRef, explicit: true };
  }
  if (requestedScope === "working-tree") {
    return { mode: "working-tree" as const, label: "working tree diff", explicit: true };
  }
  ...
  if (state.isDirty) {
    return { mode: "working-tree" as const, label: "working tree diff", explicit: false };
  }
  const detectedBase = detectDefaultBranch(cwd);
  return {
    mode: "branch" as const,
...
function collectBranchContext(cwd: string, baseRef: string, options: CollectBranchOptions = {}): BranchContext {
  const includeDiff = options.includeDiff !== false;
...
export function collectReviewContext(
  ...
  let details: WorkingTreeContext | BranchContext;
