...
class AppServerClientBase {
  ...
  handleLine(line: string): void {
    ...
    try {
      ...
      this.handleExit(
        createProtocolError(
          `Failed to parse agent app-server JSONL: ${(error as Error).message}`,
          {
            code: -32700,
        ...
        pending.reject(
          createProtocolError(
            message.error.message ??
              `agent app-server ${pending.method} failed.`,
            message.error,
          ),
        )
      } else {
  ...
  handleExit(error: Error | null): void {
    ...
    for (const pending of this.pending.values()) {
      pending.reject(
        this.exitError ?? new Error("agent app-server connection closed."),
      )
    }
    this.pending.clear()
