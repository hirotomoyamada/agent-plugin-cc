...
function logNote(message: string | null | undefined): void {
  if (!message) {
    return;
  }
...
function parseStopReviewOutput(rawOutput: unknown): { ok: boolean; reason: string | null } {
  const text = String(rawOutput ?? "").trim();
  if (!text) {
    return {
      ok: false,
      reason:
        "The stop-time Agent review task returned no final output. Run /agent:review --wait manually or bypass the gate."
    };
  }
  ...
  return {
    ok: false,
    reason:
      "The stop-time Agent review task returned an unexpected answer. Run /agent:review --wait manually or bypass the gate."
  };
}
...
function runStopReview(cwd: string, input: HookInput = {}): { ok: boolean; reason: string | null } {
  ...
  if ((result.error as NodeJS.ErrnoException | undefined)?.code === "ETIMEDOUT") {
    return {
      ok: false,
      reason:
        "The stop-time Agent review task timed out after 15 minutes. Run /agent:review --wait manually or bypass the gate."
    };
  }
  ...
  try {
    ...
    return {
      ok: false,
      reason:
        "The stop-time Agent review task returned invalid JSON. Run /agent:review --wait manually or bypass the gate."
    };
  }
}
