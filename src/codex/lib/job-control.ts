...
export const buildStatusSnapshot = (
  ...
  core.buildStatusSnapshot(CODEX_PROVIDER, cwd, {
    ...options,
...
) => core.buildSingleJobSnapshot(CODEX_PROVIDER, cwd, reference, options)

export const readStoredJob = (workspaceRoot: string, jobId: string) =>
  core.readStoredJob(CODEX_PROVIDER, workspaceRoot, jobId)

export const resolveResultJob = (
  cwd: string,
  reference: null | string | undefined,
) => core.resolveResultJob(CODEX_PROVIDER, cwd, reference)

export const resolveCancelableJob = (
  cwd: string,
  reference: null | string | undefined,
  options: SessionOptionsShim = {},
) => core.resolveCancelableJob(CODEX_PROVIDER, cwd, reference, options)
