...
function formatCodexResumeCommand(
...
function appendActiveJobsTable(
  ...
  for (const job of jobs) {
    const actions: string[] = [`/codex:status ${job.id}`]
...
export function renderJobStatusReport(job: Record<string, any>): string {
  const lines: string[] = ["# Codex Job Status", ""]
  pushJobDetails(lines, job, {
    showCancelHint: true,
...
export function renderStoredJobResult(
  ...
  const rawOutput =
    ...
    (typeof storedJob?.result?.codex?.stdout === "string" &&
...
export function renderCancelReport(job: Record<string, any>): string {
  const lines: string[] = ["# Codex Cancel", "", `Cancelled ${job.id}.`, ""]
