...
const DEFAULT_CONTINUE_PROMPT =
  "Continue from the current Kimi session and finish the requested task."

export { DEFAULT_CONTINUE_PROMPT, TASK_THREAD_PREFIX }

type ProgressCallback = ((eventOrMessage: unknown) => void) | null | undefined
...
interface RunTurnOptions {
  ...
  externalClient?: AppServerClientBase
  model?: null | string
  prompt: string
...
export function getKimiAvailability(_cwd: string): AvailabilityStatus {
  if (!binaryAvailable("kimi")) {
    return {
      available: false,
      detail:
        "`kimi` binary was not found on PATH. Install with `pip install kimi-cli`.",
    }
  }
...
export function getKimiAuthStatus(_cwd: string): AuthStatus {
  ...
  return {
    authenticated: false,
    detail:
      "No KIMI_API_KEY or MOONSHOT_API_KEY found. Run `kimi login` or export the key.",
  }
}
...
function pushContentPart(state: RunTurnState, payload: unknown): void {
...
function handleEventNotification(
  state: RunTurnState,
  notification: undefined | { params?: Record<string, unknown> },
    ...
    case "ToolCall": {
      const name =
        typeof payload?.function === "object" && payload?.function !== null
          ? ((payload.function as Record<string, unknown>).name as
              | string
              | undefined)
          : undefined
      progress?.({
...
function buildAutoApprovalResponse(
  payload: Record<string, unknown>,
): Record<string, unknown> {
  const requestId =
    typeof payload.id === "string" ? payload.id : `auto-${Date.now()}`
  return { request_id: requestId, response: "approve_for_session" }
}

function buildAutoToolErrorResponse(
  payload: Record<string, unknown>,
): Record<string, unknown> {
  const toolCallId =
    typeof payload.id === "string" ? payload.id : `auto-${Date.now()}`
  return {
    return_value: {
      is_error: true,
      message:
        "External tool calls are not supported by this Claude Code plugin.",
      output: "",
    },
...
function buildAutoQuestionResponse(
  payload: Record<string, unknown>,
): Record<string, unknown> {
  const requestId =
    typeof payload.id === "string" ? payload.id : `auto-${Date.now()}`
  ...
  for (const question of questions) {
    ...
    const firstLabel =
      ...
      typeof (options[0] as Record<string, unknown>).label === "string"
        ? ((options[0] as Record<string, unknown>).label as string)
        : ""
    if (text) {
...
function buildRejectApprovalResponse(
  payload: Record<string, unknown>,
): Record<string, unknown> {
  const requestId =
    typeof payload.id === "string" ? payload.id : `auto-${Date.now()}`
  return {
    feedback:
      "Read-only mode; tool calls that require approval are not allowed.",
...
function handleServerRequest(
  ...
  respond: (response: { error?: JsonRpcErrorLike; result?: unknown }) => void,
  progress: ProgressCallback,
  writeMode: boolean,
): void {
  ...
  const requestType =
    typeof message.params.type === "string" ? message.params.type : null
  const payload =
    message.params.payload && typeof message.params.payload === "object"
      ? (message.params.payload as Record<string, unknown>)
      : {}
...
export async function runKimiTurn(
  ...
  try {
    ...
    const status =
      result && typeof result.status === "string" ? result.status : "finished"
    if (status === "cancelled") {
...
export async function interruptKimiTurn(
  cwd: string,
...
export async function runKimiPrintMode(
  ...
  return new Promise<PrintModeResult>((resolve, reject) => {
...
export interface KimiTaskThread {
  jobId: string
  name: string
  threadId: string
  updatedAt: string
